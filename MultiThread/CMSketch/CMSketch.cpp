...
CMSketch::CMSketch(int window_sz_, int d_, int width_, int clockSize_, int _insertTimesPerUpdate)
    ...
    insertTimesPerUpdate(_insertTimesPerUpdate), updating(0), blocking(1), whileing(1)
{
    ...
    updator = thread(CMSketch::updateThread, this);
}
...
void CMSketch::Insert(int j, int flow[])
{
    this->updating = 1;
    this->blocking = 0;
    for(int k = j; k < j + insertTimesPerUpdate; ++k)
        this->insert(0, (char*)&flow[k], 1, clockSize);
    while(this->updating);
}


void CMSketch::updateClock()
{
...
void CMSketch::updateThread(CMSketch *cm)
{
    while(cm->whileing)
    {
        while(cm->whileing && cm->blocking);
        cm->updateClock();
        cm->blocking = 1;
        cm->updating = 0;
    }
